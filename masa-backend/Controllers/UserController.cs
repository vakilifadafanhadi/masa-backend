    ...
    public class UserController : ControllerBase
    {
        ...
        public async Task<ActionResult<ResponceWithData<string>>> Register(RegisterModelView request)
        {
            ...
            try
            {
                ...
                try
                {
                    var amootSms = new AmootSms();
                    string text = $"{person?.FirstName} {person?.LastName} عزیز؛ شناسه شما:\n{token}\nماسا بانک";
                    var result = await amootSms.SendMessageAsync(text, person?.Mobile!);
                    responce.Warnings.Add("پیامک ارسال شد");
                }
                catch (Exception ex)
                {
                    responce.Errors.Add("پیامک ارسال نشد");
                    responce.Add(ex);
                }
                responce.Data = token;
        ...
        public async Task<ActionResult<ResponceMV>> BusinessAccountRequest([FromRoute] Guid userId)
        {
            ...
            try
            {
                ...
                var user = _repository.UserRepository.GetUser(userId);
                user.Type = 10;
                await _repository.FileRepository.AddAsync(new FileDto
                {
                    Name = fileName,
                    UserId = userId
                });
